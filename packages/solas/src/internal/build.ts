		...
		for (const segment of res.segments) {
			try {
				...
				if (!processed.has(shellPath)) {
					prerenderCache.set(
						shellPath,
						await Prerender.getStaticFlag(shellPath, this.buildContext),
					)
				...
				for (const layoutPath of layoutPaths) {
					...
					if (!processed.has(layoutPath)) {
						prerenderCache.set(
							layoutPath,
							await Prerender.getStaticFlag(layoutPath, this.buildContext),
						)
				...
				if (shouldPrerender) {
					if (!isDynamic && !isWildcard) {
						...
						for (const r of Prerender.getDynamicRouteList(route, params, staticParams)) {
				...
				if (err instanceof Error && err.message.startsWith('Missing middleware export')) {
					throw err
				}
