...
export async function getStaticFlag(filePath: string, buildContext: BuildContext) {
	return buildContext.exportReader.literal<'full' | 'ppr' | false>(
		filePath,
		'prerender',
		(v): v is (typeof Config.PRERENDER_MODES)[number] =>
...
export async function get(
	...
	const url = `${opts.origin ?? `http://${Config.SLUG}.local`}${applyBasePath(route, opts.base)}`

	const res = await app.fetch(
		new Request(url, {
			headers: {
				Accept: 'text/html',
				[`x-${Config.SLUG}-prerender`]: '1',
				[`x-${Config.SLUG}-prerender-artifact`]: '1',
			},
		}),
	)
