...
import { BrowserRouter, withBase } from '../../../../src/internal/browser-router/shared.js'
...
describe('BrowserRouter', () => {
  describe('isHashOnlyTarget', () => {
    it('returns true for hash-only targets', () => {
      expect(BrowserRouter.isHashOnlyTarget('#section')).toBe(true)
    })

    it('returns false for non-hash targets', () => {
      expect(BrowserRouter.isHashOnlyTarget('/path')).toBe(false)
      expect(BrowserRouter.isHashOnlyTarget('https://example.com')).toBe(false)
    })
  })

  describe('isExternalTarget', () => {
    it('returns false for hash-only targets', () => {
      expect(BrowserRouter.isExternalTarget('#section', 'https://example.com')).toBe(false)
    })

    it('returns true for different origins', () => {
      expect(BrowserRouter.isExternalTarget('https://other.com', 'https://example.com')).toBe(true)
    })

    it('returns false for same origin', () => {
      expect(BrowserRouter.isExternalTarget('https://example.com/page', 'https://example.com')).toBe(false)
    })

    it('returns false for relative paths', () => {
      expect(BrowserRouter.isExternalTarget('/about', 'https://example.com')).toBe(false)
    })

    it('handles malformed URLs gracefully', () => {
      expect(BrowserRouter.isExternalTarget('', 'https://example.com')).toBe(false)
    })
  })

  describe('toTarget', () => {
    it('replaces :param segments', () => {
      const result = BrowserRouter.toTarget('/posts/:id', { id: '123' })
      expect(result).toContain('/posts/123')
    })

    it('throws for missing params', () => {
      expect(() => BrowserRouter.toTarget('/posts/:id', {})).toThrow('missing route param')
    })

    it('handles wildcard routes', () => {
      const result = BrowserRouter.toTarget('/docs/*', { wildcard0: 'guide/getting-started' })
      expect(result).toContain('/docs/guide/getting-started')
    })

    it('throws when wildcard has no remaining param', () => {
      expect(() => BrowserRouter.toTarget('/docs/*', {})).toThrow('wildcard routes require')
    })

    it('appends query params', () => {
      const result = BrowserRouter.toTarget('/search', undefined, { q: 'test' })
      expect(result).toContain('?q=test')
    })

    it('preserves existing hash', () => {
      const result = BrowserRouter.toTarget('/page#section', undefined, { q: '1' })
      expect(result).toContain('section')
    })

    it('merges query with existing query string', () => {
      const result = BrowserRouter.toTarget('/search?existing=true', undefined, { extra: '1' })
      expect(result).toContain('existing=true')
      expect(result).toContain('extra=1')
    })

    it('encodes param values', () => {
      const result = BrowserRouter.toTarget('/path/:id', { id: 'a/b' })
...
describe('ResponseCache', () => {
  ...
  describe('toCacheKey', () => {
    it('normalises a path against a base URL', () => {
      expect(ResponseCache.toCacheKey('/about?q=1', 'http://localhost')).toBe('/about?q=1')
    })

    it('strips hash from cache key', () => {
      expect(ResponseCache.toCacheKey('/about#section', 'http://localhost')).toBe('/about')
    })
