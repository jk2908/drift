...
const { BrowserRouterContext } = await import('../../../../src/internal/browser-router/router.js')
const { useRouter } = await import('../../../../src/internal/browser-router/use-router.js')

function createContextValue(overrides: Partial<BrowserRouter.Context> = {}): BrowserRouter.Context {
...
function TestComponent({ onRouter }: { onRouter: (router: BrowserRouter.Context) => void }) {
...
describe('useRouter', () => {
	...
	it('provides the go function from context', async () => {
		...
		await render(
			<BrowserRouterContext value={contextValue}>
				<TestComponent onRouter={(r) => { capturedGo = r.go }} />
			</BrowserRouterContext>,
		)
	...
	it('provides the url from context', async () => {
		...
		await render(
			<BrowserRouterContext value={contextValue}>
				<TestComponent onRouter={(r) => { capturedUrl = r.url }} />
			</BrowserRouterContext>,
		)
