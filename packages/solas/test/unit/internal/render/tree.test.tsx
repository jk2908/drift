...
function Shell({ children }: { children?: React.ReactNode }) {
	return <html lang="en"><body><div data-testid="shell">{children}</div></body></html>
}
...
describe('Tree', () => {
	...
	it('passes params to page', async () => {
		const screen = await render(
			<Tree depth={0} params={{ id: '42' }} ui={makeUI()} />,
		)
	...
	it('renders nested layouts', async () => {
		const screen = await render(
			<Tree
				depth={1}
				params={{}}
				ui={makeUI({ layouts: [Shell, Layout] })}
			/>,
		)
