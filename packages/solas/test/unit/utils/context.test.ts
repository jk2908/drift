...
describe('Context', () => {
	describe('create', () => {
		it('creates a context with use and write methods', () => {
			const ctx = Context.create<{ value: string }>('test')
	...
	describe('write', () => {
		it('provides context to the callback', async () => {
			const ctx = Context.create<number>('counter')
			const result = await ctx.write(42, () => ctx.use())
			expect(result).toBe(42)
		})

		it('supports async callbacks', async () => {
			const ctx = Context.create<string>('async')
		...
		it('isolates nested contexts', async () => {
			const ctx = Context.create<number>('nested')
		...
		it('context is not available after write completes', async () => {
			const ctx = Context.create<string>('scoped')
