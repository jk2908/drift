...
describe('isKnownError', () => {
	...
	it('returns true for render abort message', () => {
		expect(isKnownError('The render was aborted by the server without a reason')).toBe(
			true,
		)
	})
	...
	it('returns true for object with render abort message', () => {
		expect(
			isKnownError({ message: 'The render was aborted by the server without a reason' }),
		).toBe(true)
	})
